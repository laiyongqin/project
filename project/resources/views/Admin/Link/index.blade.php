     ...
     <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info"> 
      ...
      <tbody role="alert" aria-live="polite" aria-relevant="all">
       ...
       <tr class="odd"> 
        ...
        <td class=" ">{{$row->yqname}}</td> 
        <td class=" ">{{$row->yq_url}}</td> 
        <td class=" ">
           @if ( $row->status ) 
                上架
                @else
        ...
        <td class=" ">
          <a href="/adminlink/create" class="btn btn-success">添加</a>
          <form action="/adminlink/{{$row->id}}" method="post">
            {{csrf_field()}}
            {{method_field("DELETE")}}
            <button class="btn btn-info">删除</button>
          </form>
        <a href="/adminlink/{{$row->id}}/edit" class="btn btn-danger">修改</a></td> 
       </tr>
       @endforeach
      </tbody>
     </table>
...
$(".del").click(function(){
  ...
  // alert(a);
  //Ajax操作
  $.get("/adminlink/{delete}",{a:a},function(data){
    // alert(data);
    if(data==1){      
