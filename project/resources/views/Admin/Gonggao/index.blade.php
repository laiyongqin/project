     ...
     <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info"> 
      <thead> 
       <tr role="row">
        ...
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Platform(s): activate to sort column ascending" style="width: 191px;">链接url</th>
        <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="CSS grade: activate to sort column ascending" style="width: 101px;">操作</th>
       </tr> 
      </thead> 
      <tbody role="alert" aria-live="polite" aria-relevant="all">
        @foreach($gonggao as $row)
       <tr class="odd"> 
         <td class="checkbox-column"><input type="checkbox" value="{{$row->gid}}"> </td>
        <td class="sorting_1">{{$row->gid}}</td> 
        <td class=" ">{{$row->gname}}</td> 
        <td class="gcontent">{{$row->gcontent}}</td> 

        <td class=" ">
          <a href="/admingonggao/create" class="btn btn-success">添加</a>
          <form action="/admingonggao/{{$row->gid}}" method="post">
            {{csrf_field()}}
            {{method_field("DELETE")}}
            <button class="btn btn-info">删除</button>
          </form>
        <a href="/admingonggao/{{$row->gid}}/edit" class="btn btn-danger">修改</a>
        <a href="/admingonggao/{{$row->gid}}/edit?info=info" class="btn btn-info">详情</a></td> 
       </tr>
       @endforeach
      </tbody>
     </table>
...
$(".del").click(function(){
  ...
  // alert(a);
  //Ajax操作
  $.get("/admingonggao/{delete}",{a:a},function(data){
    // alert(data);
    if(data==1){      
